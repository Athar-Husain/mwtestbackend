...
export const createTicketdum = async (req, res) => {
  try {
    ...
    // Step 2: Fetch customer and service area
    const customer =
      await Customer.findById(customerId).populate('serviceArea');
    if (!customer) {
      return res.status(404).json({ message: 'Customer not found' });
    ...
    // Step 3: Assign to a team member based on service area
    const assignedTeamMember = await Team.findOne({ area: serviceAreaId });
    if (!assignedTeamMember) {
      return res
        .status(404)
    ...
    // Step 4: Determine who is creating the ticket
    const createdBy = requester._id;
    const createdByModel = requester.role; // 'Customer', 'Admin', or 'Team'
...
export const assignTicketold2 = async (req, res) => {
  try {
    ...
    ticket.assignedTo = teamMember._id;
    ticket.assignmentHistory.push({
      assignedTo: teamMember._id,
      assignedAt: new Date(),
      reassignedBy: req.user._id,
      reassignedByModel: req.user.role === 'admin' ? 'Admin' : 'TeamMember',
      note,
    });

    ticket.updatedBy = req.user._id;
    ticket.updatedByModel = req.user.role === 'admin' ? 'Admin' : 'TeamMember';
...
export const getTicketsByCustomer = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to get tickets by customer', error });
  }
};
...
// import SupportTicket from './models/SupportTicket'; // Assuming you have this model
// import Connection from './models/Connection';

export const createTicketForCustomer = async (req, res) => {
  try {
    // Fetch the customer details and their active connection
    ...
    if (!customer || !customer.activeConnection) {
      return res
        .status(404)
        .json({ message: 'Customer or active connection not found' });
    }

    const activeConnection = customer.activeConnection;
    const serviceArea = activeConnection.serviceArea;

    // Create the ticket, and assign it to the relevant service area
    const newTicket = new SupportTicket({
      customerId: customer._id,
      connectionId: activeConnection._id,
      serviceArea: serviceArea._id, // Associate the ticket with the service area
      priority: req.body.priority || 'medium', // Set ticket priority
      description: req.body.description,
      status: 'open', // Set the initial ticket status
    });

    await newTicket.save();

    // You can also add real-time notifications or any other logic here

...
export const addPublicComment = async (req, res) => {
  try {
    ...
    console.log('error in addpublic message', error.message);
    res
      .status(500)
...
export const addPrivateComment = async (req, res) => {
  try {
    const { ticketId } = req.params;
    const { content } = req.body; // Ensure only an Admin or TeamMember can add a private note
...
const createAttachment = async (file) => {
  ...
  // For a local setup, the file's path is created by a middleware like multer.
...
export const addAttachmentToTicket = async (req, res) => {
  try {
    const { ticketId } = req.params;
    const file = req.file; // Assuming single file upload middleware
